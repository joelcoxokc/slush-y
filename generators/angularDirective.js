module.exports = function (gulp, install, conflict, template, rename, _, inflections, inquirer, mkdirp) {
	...
	gulp.task('angular-directive', function (done) {
		...
		if (!this.args[0]) {
			...
			console.log('******    Ex: slush meanjs:angular-directive books                ******');
			return done();
		}
		...
		inquirer.prompt(prompts,
			function (answers) {
				...
				gulp.src(__dirname + '/../templates/angular-directive/_.client.directive.js')
			        ...
			        .pipe(conflict('public/modules/' + answers.slugifiedModuleName + '/directives/'))
			        .pipe(gulp.dest('public/modules/' + answers.slugifiedModuleName + '/directives/'))
			        .on('end', function () {
		               done();
		            });	
			});
