(function(){
  ...
  function Generator(name, options){
    ...
    this.emitArgs = function(args){
      _.forEach(args, function (arg, key){
        if(key === '_'){
          return;
        }
        // console.log(_this.options[key]);
        emitter.emit(_this.name + ':'+key, arg);
      })
    }
  }
  ...
  Generator.prototype.run = function(name, options){
    ...
    _this.cwd.app = path.join(process.cwd(), 'src/js/app');
  ...
  Generator.prototype.action = function(flags, description, fn, defaultValue){
    var _this = this
    ...
    console.log(option.is(option.short));
    emitter.on(_this.name+':'+oname, function (val){
      // if(_.isArray( val )){
        // _this.flags[name] = val;
      // }\
      console.log(_this.flags);
      _this.flags[name] = val;
    })
  }
